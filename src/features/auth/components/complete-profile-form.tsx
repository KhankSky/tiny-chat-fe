...
export function CompleteProfileForm({ dictionary }: { dictionary: Dictionary }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      sessionStorage.removeItem(DRAFT_KEY); setCompleted(true);
    } catch (err) { setAvatarUploading(false); setError(err instanceof Error ? err.message : t.saveProfileError); } finally { setLoading(false); }
  }

  if (completed) return <section className="space-y-6 text-center"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-cyan-400 text-3xl text-slate-950">✓</div><h2 className="text-2xl font-semibold">{t.profileReadyTitle}</h2><p className="text-slate-300">{t.profileReadyDescription}</p><div className="flex flex-col gap-3 sm:flex-row"><button type="button" onClick={() => router.push("/groups/match")} className="min-h-11 flex-1 rounded-full bg-cyan-400 px-5 font-semibold text-slate-950">{t.findMatchingGroupButton}</button><button type="button" onClick={() => setCompleted(false)} className="min-h-11 flex-1 rounded-full border border-white/15 px-5 font-semibold text-white">{t.reviewProfileButton}</button></div></section>;
