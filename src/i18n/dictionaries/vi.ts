export const vi = {
  appName: "Conyva",
  appTagline: "Nói chuyện, học tập, kết nối",
  seo: {
    homeTitle: "Conyva – Luyện tiếng Anh qua chat mỗi ngày theo nhóm nhỏ",
    socialHeadline: "Luyện tiếng Anh mỗi ngày qua những cuộc chat thật.",
    socialDescription: "Chat cùng nhóm nhỏ phù hợp với trình độ, mục tiêu và sở thích của bạn.",
    socialImageAlt: "Conyva: Luyện tiếng Anh qua chat mỗi ngày theo nhóm nhỏ",
  },
  common: {
    logout: "Đăng xuất",
    anonymous: "Ẩn danh",
    cancel: "Hủy",
    close: "Đóng",
    completeProfile: "Hoàn thiện hồ sơ",
    loading: "Đang tải...",
    loggedIn: "Đã đăng nhập",
    saveChanges: "Lưu thay đổi",
    saving: "Đang lưu...",
    userFallback: "Người dùng",
    you: "Bạn",
  },
  header: {
    nav: [
      { label: "Tính năng", href: "#features" },
      { label: "Cách hoạt động", href: "#how-it-works" },
      { label: "Liên hệ", href: "#contact" },
    ],
    login: "Đăng nhập",
    register: "Đăng ký",
    switchLabel: "EN",
    backToChat: "Quay lại trang chat",
  },
  landing: {
    badge: "Nhóm nhỏ • Ghép phù hợp • Duy trì streak",
    title: "Luyện tiếng Anh mỗi ngày qua những cuộc trò chuyện thật.",
    description:
      "Conyva ghép bạn vào nhóm nhỏ phù hợp với trình độ, mục tiêu và sở thích, để bạn luôn có người cùng luyện.",
    primaryCta: "Tìm nhóm để luyện ngay",
    completeProfileCta: "Hoàn thiện hồ sơ",
    findGroupCta: "Tìm nhóm phù hợp",
    returnToChatCta: "Quay lại cuộc trò chuyện",
    secondaryCta: "Cách Conyva hoạt động",
    previewTitle: "Xem trước",
    previewGroupName: "English Conversation — A2",
    previewGroupMeta: "4 thành viên · Đang hoạt động hôm nay",
    previewStreak: "Streak nhóm · 5 ngày",
    previewJoinCta: "Tìm nhóm",
    bubbles: [
      { label: "Minh", text: "I had a great coffee with my friend this morning." },
      { label: "Lan", text: "That sounds nice! I enjoyed a walk after work." },
      { label: "Bạn", text: "The best part was cooking dinner with my family." },
    ],
    trustTitle: "Một cách nhẹ nhàng hơn để bắt đầu nói.",
    trustItems: [
      "Nhóm nhỏ, dễ bắt đầu nói",
      "Phù hợp nhiều trình độ",
      "Không gian thân thiện để bắt đầu nói",
      "Luyện đều cùng nhau mỗi ngày",
    ],
    benefitsLabel: "Conyva giúp gì cho bạn?",
    benefitsTitle: "Mọi thứ bạn cần để biến việc nói thành thói quen mỗi ngày.",
    features: [
      {
        title: "Tìm đúng người để luyện",
        description:
          "Ghép với những người cùng trình độ, mục tiêu và sở thích thay vì bị lạc trong một nhóm đông.",
      },
      {
        title: "Bắt đầu trò chuyện tự nhiên",
        description:
          "Nhóm nhỏ thân thiện giúp bạn dễ chào hỏi, chia sẻ và duy trì cuộc trò chuyện.",
      },
      {
        title: "Cùng nhau duy trì thói quen",
        description:
          "Hoạt động nhóm và streak giúp bạn quay lại luyện tập đều đặn hơn.",
      },
    ],
    foundationLabel: "Cách Conyva hoạt động",
    foundationTitle: "Bắt đầu luyện tập qua ba bước đơn giản.",
    steps: [
      { title: "Cho Conyva biết về bạn", description: "Chọn trình độ, mục tiêu và sở thích." },
      { title: "Tham gia nhóm nhỏ", description: "Conyva tìm những người phù hợp với bạn." },
      { title: "Bắt đầu trò chuyện", description: "Làm quen với nhóm và luyện cùng nhau." },
    ],
    fitLabel: "Conyva có hợp với bạn không?",
    fitTitle: "Dành cho người muốn nói nhưng chưa biết bắt đầu từ đâu.",
    fitDescription:
      "Conyva phù hợp nếu bạn muốn luyện giao tiếp tiếng Anh nhưng ngại bắt đầu, không có bạn cùng luyện, hoặc thường bỏ dở sau vài ngày.",
  },
  auth: {
    loginTitle: "Chào mừng quay lại",
    registerTitle: "Tạo tài khoản của bạn",
    loginDescription:
      "Đăng nhập để tiếp tục cuộc trò chuyện, lấy lại hồ sơ và đi vào ứng dụng chính.",
    registerDescription:
      "Tạo tài khoản Conyva trước. Phần hoàn thiện hồ sơ được tách riêng để luồng auth luôn nhanh và gọn.",
    completeProfileTitle: "Hoàn thiện hồ sơ",
    completeProfileDescription:
      "Thông tin này giúp Conyva ghép bạn với nhóm phù hợp hơn.",
    progressLabel: "Tiến độ onboarding",
    stepLabel: "Bước",
    backButton: "Quay lại",
    continueButton: "Tiếp tục",
    practiceGoalStepTitle: "Mục tiêu luyện tập",
    practiceGoalStepDescription: "Chọn kết quả chính bạn mong muốn khi luyện tiếng Anh.",
    englishLevelStepTitle: "Trình độ tiếng Anh",
    englishLevelStepDescription: "Chọn mô tả gần nhất với khả năng hiện tại của bạn.",
    interestsStepTitle: "Sở thích của bạn",
    interestsStepDescription: "Chọn chủ đề giúp bạn dễ bắt đầu trò chuyện hơn.",
    profileStepTitle: "Hồ sơ cá nhân",
    profileStepDescription: "Thêm thông tin mà những người cùng nhóm có thể nhìn thấy.",
    reviewStepTitle: "Xem lại hồ sơ",
    reviewStepDescription: "Kiểm tra thông tin trước khi hoàn tất hồ sơ.",
    profileReadyTitle: "Hồ sơ của bạn đã sẵn sàng",
    profileReadyDescription: "Bạn có thể bắt đầu tìm nhóm phù hợp với mục tiêu và sở thích.",
    findMatchingGroupButton: "Tìm nhóm phù hợp với tôi",
    reviewProfileButton: "Xem lại hồ sơ",
    noBioLabel: "Chưa thêm giới thiệu",
    emailLabel: "Email",
    passwordLabel: "Mật khẩu",
    confirmPasswordLabel: "Nhập lại mật khẩu",
    passwordMismatch: "Mật khẩu nhập lại không khớp.",
    displayNameLabel: "Tên hiển thị",
    avatarUrlLabel: "Ảnh đại diện",
    avatarUploadHint: "Bấm để tải ảnh lên",
    englishLevelLabel: "Trình độ tiếng Anh",
    practiceGoalLabel: "Mục tiêu luyện tập",
    interestsLabel: "Sở thích",
    shortBioLabel: "Giới thiệu ngắn",
    emailPlaceholder: "ban@example.com",
    passwordPlaceholder: "••••••••",
    displayNamePlaceholder: "Nguyễn An",
    bioPlaceholder: "Mình muốn luyện nói tiếng Anh mỗi ngày.",
    loading: "Vui lòng chờ...",
    logout: "Đăng xuất",
    loginButton: "Đăng nhập",
    googleAuthTitle: "Đăng nhập nhanh với Google",
    googleAuthDescription: "Nhanh chóng và an toàn với tài khoản Google",
    authDivider: "Hoặc",
    registerButton: "Tạo tài khoản",
    registerSuccess: "Đăng ký thành công. Vui lòng đăng nhập để tiếp tục.",
    completeProfileButton: "Hoàn tất hồ sơ",
    backToLanding: "Về trang chủ",
    pickInterestError: "Chọn ít nhất một sở thích.",
    englishLevelPlaceholder: "Chọn trình độ của bạn",
    practiceGoalPlaceholder: "Chọn mục tiêu chính",
    englishLevelRequired: "Vui lòng chọn trình độ tiếng Anh.",
    practiceGoalRequired: "Vui lòng chọn mục tiêu luyện tập.",
    displayNameRequired: "Hãy nhập tên hiển thị có ít nhất 2 ký tự.",
    bioTooLong: "Giới thiệu không được vượt quá 200 ký tự.",
    avatarTypeError: "Hãy chọn ảnh JPG, PNG hoặc WebP.",
    avatarSizeError: "Ảnh đại diện phải có dung lượng tối đa 5 MB.",
    avatarRemove: "Xóa ảnh đại diện",
    avatarUploading: "Đang tải ảnh đại diện...",
    interestsCountLabel: "Sở thích đã chọn",
    interestsMinError: "Hãy chọn ít nhất 3 sở thích.",
    interestsMaxError: "Bạn chỉ có thể chọn tối đa 5 sở thích.",
    availabilityLabel: "Bạn thường rảnh vào thời gian nào?",
    timezoneLabel: "Múi giờ",
    frequencyLabel: "Bạn muốn luyện tập với tần suất nào?",
    saveProfileError: "Không thể lưu hồ sơ",
    errorFallback: "Gửi yêu cầu thất bại",
  },
  chat: {
    conversationsTitle: "Đoạn chat",
    conversationsDescription: "Chọn một cuộc hội thoại để vào nhắn tin.",
    loadConversationsError: "Không thể tải danh sách chat",
    needGroupTitle: "Cần một nhóm mới?",
    needGroupDescription: "Ghép nhóm theo level, mục tiêu và sở thích chỉ với một nút bấm.",
    findGroup: "Tìm nhóm",
    findNewGroup: "Tìm nhóm mới",
    noConversations: "Bạn chưa có cuộc trò chuyện nào.",
    noMessages: "Chưa có tin nhắn.",
    selectConversationTitle: "Chọn một cuộc trò chuyện",
    selectConversationDescription:
      "Danh sách bên trái được lấy từ backend theo tài khoản đang đăng nhập. Khi mở một cuộc trò chuyện, phần giữa sẽ thành khung chat chính, còn sidebar phải có thể thu gọn giống Messenger.",
    layoutTitle: "Bố cục",
    layoutDescription: "Sidebar trái, nội dung giữa, sidebar phải.",
    focusTitle: "Tập trung",
    focusDescription: "Không còn khung giữa bị bó hẹp, giao diện sẽ trải ngang hơn.",
    quickTipTitle: "Mẹo nhanh",
    quickTipDescription:
      "Dùng danh sách bên trái để chuyển cuộc hội thoại, rồi thu gọn sidebar phải nếu muốn vùng chat rộng hơn.",
    shortcutsEyebrow: "Lối tắt",
    workspaceTitle: "Workspace",
    workspaceDescription: "Các thao tác thường dùng để chuyển nhanh sang những luồng chính.",
    completeProfileTitle: "Hoàn thiện hồ sơ",
    completeProfileDescription: "Tối ưu ghép nhóm và trải nghiệm chat.",
    sidebarTitle: "Đoạn chat",
    sidebarDescription: "Các cuộc trò chuyện nhóm của bạn nằm ở đây để chuyển nhanh.",
    personalInfo: "Thông tin cá nhân",
    roomEyebrow: "Phòng trò chuyện",
    personalStreak: "Streak cá nhân",
    streakDays: "ngày",
    messagesToday: "Tin nhắn hôm nay",
    dailyTopicTitle: "Topic hôm nay",
    dailyTopicAction: "Trả lời",
    dailyTopicReplying: "Đang trả lời topic",
    dailyTopicQuotePrefix: "Topic hôm nay:",
    loadDailyTopicError: "Không thể tải topic hôm nay",
    loadStreakError: "Không thể tải streak",
    streakUnavailable: "Chưa có streak",
    realtimeSignInRequired: "Bạn cần đăng nhập để chat realtime.",
    invalidChatData: "Nhận dữ liệu chat không hợp lệ.",
    socketStatus: {
      connected: "Đang realtime",
      connecting: "Đang kết nối",
      error: "Mất kết nối",
      idle: "Chưa sẵn sàng",
    },
    typingSingle: "{name} đang nhập...",
    typingDouble: "{first} và {second} đang nhập...",
    typingMany: "{name} và những người khác đang nhập...",
    onlineNow: "Trực tuyến",
    offlineNow: "Ngoại tuyến",
    onlineCount: "{count} người online",
    seenByCount: "Đã xem bởi {count}",
    toggleSidebar: {
      collapse: "Thu gọn thông tin",
      open: "Mở thông tin",
    },
    waitingForConnection: "Đang chờ kết nối...",
    writeMessage: "Nhập tin nhắn...",
    send: "Gửi",
    loadMessagesError: "Không thể tải tin nhắn",
    sendMessageError: "Không thể gửi tin nhắn",
    emoji: "Chọn emoji",
    conversationUnavailable: "Không thể mở cuộc trò chuyện",
    backToConversations: "Quay lại danh sách trò chuyện",
    unknownSocketError: "Lỗi không xác định",
    friends: {
      friendsTitle: "Bạn bè",
      noFriends: "Chưa có bạn bè",
      loadFriendsError: "Không thể tải danh sách bạn bè",
      actionError: "Không thể cập nhật thao tác kết bạn",
      sentRequest: "Đã gửi lời mời kết bạn",
      accept: "Chấp nhận",
      reject: "Từ chối",
      openChat: "Chat",
      memberProfileTitle: "Hồ sơ thành viên",
      loadProfileError: "Không thể tải hồ sơ",
      levelLabel: "Trình độ tiếng Anh",
      goalLabel: "Mục tiêu luyện tập",
      unknownValue: "Chưa rõ",
      interestsLabel: "Sở thích",
      noInterests: "Chưa có sở thích",
      bioLabel: "Giới thiệu",
      noBio: "Chưa có giới thiệu",
      addFriend: "Kết bạn",
      requestSent: "Đã gửi lời mời",
      selfProfile: "Đây là bạn",
      activeNow: "Đang hoạt động",
      activeMinutesAgo: "Hoạt động {minutes} phút trước",
      activeHoursAgo: "Hoạt động {hours} giờ trước",
      activeDaysAgo: "Hoạt động {days} ngày trước",
      activeUnknown: "Không hoạt động",
    },
    groupSidebar: {
      directChatTitle: "Trò chuyện riêng",
      partnerLabel: "Bạn trò chuyện",
      onlineStatusShown: "Trạng thái trực tuyến hiển thị trong chat",
      levelLabel: "Trình độ",
      goalLabel: "Mục tiêu",
      interestsLabel: "Sở thích",
      bioLabel: "Giới thiệu",
      collapsedLabel: "Thành viên",
      openRightSidebar: "Mở sidebar phải",
      collapseRightSidebar: "Thu gọn sidebar phải",
      membersTitle: "Thành viên",
      membersEyebrow: "Thành viên nhóm",
      loadingGroup: "Đang tải nhóm",
      groupIdLabel: "Group ID",
      online: "Trực tuyến",
      offline: "Ngoại tuyến",
      loadGroupError: "Không thể tải nhóm",
      editGroup: "Sửa nhóm",
      groupAvatarLabel: "Avatar nhóm",
      groupAvatarUploadHint: "Bấm để tải ảnh lên",
      groupNamePlaceholder: "Tên nhóm",
      groupDescriptionPlaceholder: "Mô tả nhóm",
      saveGroupError: "Không thể lưu nhóm",
      leaveGroup: "Rời nhóm",
      leaveConfirmTitle: "Rời nhóm này?",
      leaveConfirm: "Bạn sẽ rời cuộc trò chuyện này và có thể tìm một nhóm mới bất cứ lúc nào.",
      leaveError: "Không thể rời nhóm",
      groupStreakTitle: "Streak nhóm",
      currentStreak: "Streak",
      activeToday: "Đã luyện hôm nay",
      messagesToday: "Tin nhắn",
      loadStreakError: "Không thể tải streak",
      streakUnavailable: "Chưa có streak",
      groupStreakCounted: "Tuyệt, hôm nay cả nhóm đã cùng luyện.",
      groupStreakNeedsMembers: "Rủ thêm một bạn viết hôm nay để giữ lửa.",
    },
    profileModal: {
      ariaLabel: "Chỉnh sửa thông tin cá nhân",
      title: "Sửa thông tin cá nhân",
      avatarAlt: "Avatar cá nhân",
      avatarUploadHint: "Bấm avatar để tải ảnh lên",
      displayNameLabel: "Tên hiển thị",
      displayNamePlaceholder: "Ví dụ: Sky",
      languageLabel: "Ngôn ngữ",
      languageEnglish: "Tiếng Anh",
      languageVietnamese: "Tiếng Việt",
      themeLabel: "Giao diện",
      themeDark: "Tối",
      themeLight: "Sáng",
      loadError: "Không thể tải hồ sơ",
      saveError: "Không thể lưu hồ sơ",
    },
  },
  groups: {
    metadataTitle: "Tìm nhóm",
    metadataDescription: "Ghép vào nhóm nhỏ phù hợp theo trình độ, mục tiêu và sở thích.",
    eyebrow: "Ghép nhóm",
    title: "Tìm một nhóm nhỏ để vào cuộc trò chuyện dễ hơn.",
    description:
      "Hệ thống ưu tiên ghép theo trình độ trước, sau đó tối ưu theo mục tiêu và sở thích chung. Nhóm có thể hoạt động ngay khi chưa đủ 5 người và luôn bị giới hạn ở 5 thành viên.",
    primary: "Tìm nhóm",
    secondary: "Về danh sách chat",
    profileReady: "Hồ sơ đã sẵn sàng",
    profileMissing: "Hoàn thiện hồ sơ trước",
    profileHint:
      "Hồ sơ đầy đủ giúp hệ thống ghép chính xác hơn, nhất là ở trình độ, mục tiêu và sở thích.",
    profileChecklistTitle: "Tín hiệu hồ sơ",
    readyLabel: "Sẵn sàng",
    improveLabel: "Bổ sung",
    statusIdle: "Sẵn sàng khi bạn muốn",
    statusLoading: "Đang tìm nhóm phù hợp nhất",
    loadingHint:
      "Mình đang kiểm tra trình độ, chỗ trống còn lại và độ khớp về mục tiêu/sở thích.",
    actionPanelTitle: "Phòng ghép",
    actionPanelHeading: "Bắt đầu tìm nhóm",
    actionPanelHint: "Bấm một lần, hệ thống sẽ tìm phòng phù hợp với hồ sơ của bạn.",
    howItWorksTitle: "Cách ghép",
    rulesSummary: "Quy tắc ghép nhóm",
    matchingSteps: [
      "Kiểm tra hồ sơ của bạn",
      "Tìm người học gần trình độ",
      "Vào phòng hoặc tạo phòng mới",
    ],
    successTitle: "Bạn đã vào nhóm",
    currentGroupTitle: "Nhóm hiện tại của bạn",
    alreadyInGroup: "Bạn đang ở trong một nhóm",
    currentGroupBadge: "Nhóm hiện tại",
    noNewGroupTitle: "Chưa tìm thấy nhóm mới",
    noNewGroup: "Bạn vẫn đang ở nhóm hiện tại",
    noNewGroupBadge: "Chưa có match mới",
    openChat: "Mở chat",
    searchAgain: "Tìm lại nhóm",
    redirectingToChat: "Đã ghép nhóm thành công. Sẽ chuyển vào cuộc trò chuyện sau {seconds} giây...",
    newGroup: "Đã tạo nhóm mới",
    joinedGroup: "Đã vào nhóm hiện có",
    sizeLabel: "Số người trong nhóm",
    levelLabel: "Trình độ mục tiêu",
    goalLabel: "Mục tiêu khớp",
    interestsLabel: "Sở thích chung",
    reasonLabel: "Lý do ghép",
    joinedAtLabel: "Thời điểm tham gia",
    bestPracticeTitle: "Quy tắc ghép nhóm",
    rules: [
      "Mỗi nhóm tối đa 5 người.",
      "Từ 3 đến 5 người là mức phù hợp nhất cho thảo luận.",
      "Ưu tiên người cùng hoặc gần trình độ tiếng Anh.",
      "Mục tiêu và sở thích gần nhau giúp cuộc trò chuyện tự nhiên hơn.",
      "Ở MVP, mỗi user chỉ nên ở một số nhóm nhỏ và có chủ đích.",
    ],
    supportTitle: "Sau khi bấm tìm nhóm",
    supportText:
      "Nếu có nhóm còn chỗ trống, bạn sẽ vào ngay nhóm đó. Nếu chưa có nhóm phù hợp, hệ thống sẽ tạo nhóm mới để bạn không phải chờ.",
    noProfile: "Chưa có hồ sơ",
    completeProfileNow: "Hoàn thiện hồ sơ ngay",
    levelUnknown: "Trình độ chưa có",
    goalUnknown: "Mục tiêu chưa có",
    interestsCount: "sở thích",
    noInterests: "Chưa có sở thích",
    signalsTitle: "Tín hiệu dùng để ghép",
    signals: [
      "Cùng hoặc gần level",
      "Sở thích giao nhau",
      "Mục tiêu học tập giống nhau",
      "Chừa slot cho nhóm tối đa 5 người",
    ],
    matchErrorTitle: "Không thể ghép nhóm",
    matchErrorFallback: "Không thể ghép nhóm",
    maxActiveGroupsError: "Bạn đã tham gia tối đa số nhóm đang hoạt động. Hãy rời một nhóm trước khi tìm nhóm mới.",
    yourGroup: "Nhóm của bạn",
  },
  profile: {
    title: "Hồ sơ cá nhân",
    description: "Cập nhật tên hiển thị và avatar dùng trong Conyva.",
    loadError: "Không thể tải hồ sơ",
    saveError: "Không thể lưu hồ sơ",
    userIdLabel: "User ID",
    displayNameLabel: "Tên hiển thị",
    displayNamePlaceholder: "Khanh",
    avatarUrlLabel: "Ảnh đại diện",
    avatarLabel: "Ảnh đại diện",
    avatarUploadHint: "Bấm để tải ảnh lên",
    saveButton: "Lưu hồ sơ",
  },
  feedback: {
    ariaLabel: "Gửi góp ý",
    entryLabel: "Gửi góp ý hoặc báo lỗi",
    title: "Gửi góp ý",
    description: "Cho tụi mình biết điều gì bị lỗi, khó hiểu hoặc nên cải tiến.",
    typeLabel: "Phản hồi này về gì?",
    severityLabel: "Mức độ ảnh hưởng?",
    messageLabel: "Chuyện gì đã xảy ra?",
    messagePlaceholder: "Mô tả lỗi hoặc ý tưởng. Trang hiện tại và thông tin thiết bị sẽ được gửi kèm tự động.",
    messageRequired: "Nhập nội dung góp ý trước nhé.",
    contextHint: "Tụi mình sẽ gửi kèm trang hiện tại và thông tin thiết bị để đội sản phẩm kiểm tra nhanh hơn.",
    submit: "Gửi góp ý",
    submitting: "Đang gửi...",
    submitError: "Không thể gửi góp ý",
    success: "Cảm ơn bạn. Phản hồi đã được ghi nhận.",
    types: {
      BUG: "Báo lỗi",
      IMPROVEMENT: "Cải tiến",
      UI_UX: "Giao diện",
      CHAT_GROUP: "Chat/nhóm",
      ACCOUNT: "Tài khoản",
      OTHER: "Khác",
    },
    severities: {
      LOW: "Nhẹ",
      MEDIUM: "Khó chịu",
      BLOCKING: "Chặn sử dụng",
    },
  },
  enums: {
    englishLevel: {
      LEVEL_A: "Người mới bắt đầu — hiểu từ và câu đơn giản",
      LEVEL_B: "Cơ bản–trung cấp — trò chuyện về chủ đề quen thuộc",
      LEVEL_C: "Khá — diễn đạt ý kiến và duy trì hội thoại",
    },
    englishLevelDescription: {
      LEVEL_A: "Tôi hiểu được từ và câu tiếng Anh đơn giản.",
      LEVEL_B: "Tôi có thể trò chuyện về các chủ đề quen thuộc.",
      LEVEL_C: "Tôi có thể diễn đạt ý kiến và duy trì hội thoại.",
    },
    practiceGoal: {
      DAILY_CHAT: "Trò chuyện hằng ngày",
      IMPROVE_WRITING: "Cải thiện cách viết",
      MAKE_FRIENDS: "Tự tin giao tiếp và kết bạn",
      TOEIC_BASIC: "TOEIC cơ bản",
      IELTS_BASIC: "IELTS cơ bản",
    },
    practiceGoalDescription: {
      DAILY_CHAT: "Tạo thói quen qua những cuộc trò chuyện hằng ngày.",
      IMPROVE_WRITING: "Viết rõ ràng và tự nhiên hơn.",
      MAKE_FRIENDS: "Gặp gỡ mọi người và tự tin kết nối hơn.",
      TOEIC_BASIC: "Làm quen với những nền tảng TOEIC cơ bản.",
      IELTS_BASIC: "Làm quen với những nền tảng IELTS cơ bản.",
    },
    availability: { MORNING: "Buổi sáng", AFTERNOON: "Buổi chiều", EVENING: "Buổi tối", LATE_NIGHT: "Đêm muộn", WEEKEND: "Cuối tuần" },
    frequency: { CASUAL: "Khi rảnh", FEW_TIMES_A_WEEK: "Vài lần mỗi tuần", ALMOST_DAILY: "Gần như mỗi ngày" },
    frequencyDescription: { CASUAL: "Luyện tập theo lịch linh hoạt.", FEW_TIMES_A_WEEK: "Duy trì nhịp luyện tập hằng tuần.", ALMOST_DAILY: "Luyện tập gần như mỗi ngày." },
    interest: {
      FOOD: "Ẩm thực",
      TRAVEL: "Du lịch",
      STUDY: "Học tập",
      WORK: "Công việc",
      MUSIC: "Âm nhạc",
      MOVIES: "Phim ảnh",
      DAILY_LIFE: "Cuộc sống",
      SPORT: "Thể thao",
      TECHNOLOGY: "Công nghệ",
      BOOKS: "Sách",
      GAMES: "Trò chơi",
    },
  },
};
